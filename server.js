// ==================== OKSAGRAM FULL SERVER ====================
// Все оригинальные функции: каналы, школы, сообщества, тематы, посты, стикеры, избранное
// Шифрование AES-256-GCM, регистрация, личные чаты
...
// ============ БАЗА ДАННЫХ (JSON) ============
let users = {};           // userId -> { id, username, passwordHash, avatar, createdAt, lastSeen, theme, globalNickname }
let chats = {};           // chatId -> { id, type, name, participants, encryptedKeys, createdAt, avatar, description, adminCode, ownerCode, permissions, public, teacher, students, currentQuiz, members, chats, pinnedPosts }
let messages = {};        // chatId -> [ { id, encrypted, senderId, senderName, timestamp, type, content?, fileName?, fileSize?, reactions?, comments?, important? } ]
let globalStickers = [];  // [ { id, url, addedBy, addedAt } ]
let favorites = { chats: {}, messages: {}, stickers: {} }; // userId -> { chats: [], messages: [], stickers: [] }
...
function loadData() {
    try {
        ...
        const stickersFile = path.join(DATA_DIR, 'globalStickers.json');
        const favoritesFile = path.join(DATA_DIR, 'favorites.json');
        const bufferFile = path.join(DATA_DIR, 'forwardBuffer.json');
        
        if (fs.existsSync(usersFile)) users = JSON.parse(fs.readFileSync(usersFile, 'utf8'));
        if (fs.existsSync(chatsFile)) chats = JSON.parse(fs.readFileSync(chatsFile, 'utf8'));
        if (fs.existsSync(messagesFile)) messages = JSON.parse(fs.readFileSync(messagesFile, 'utf8'));
        if (fs.existsSync(stickersFile)) globalStickers = JSON.parse(fs.readFileSync(stickersFile, 'utf8'));
        if (fs.existsSync(favoritesFile)) favorites = JSON.parse(fs.readFileSync(favoritesFile, 'utf8'));
        if (fs.existsSync(bufferFile)) forwardBuffer = JSON.parse(fs.readFileSync(bufferFile, 'utf8'));
...
function generateToken(userId) {
    return jwt.sign({ userId, exp: Math.floor(Date.now() / 1000) + (7 * 24 * 60 * 60) }, JWT_SECRET);
}
function verifyToken(token) {
    try { return jwt.verify(token, JWT_SECRET).userId; } catch(e) { return null; }
}
function sanitizeInput(str, max = 500) { return str ? str.replace(/[<>]/g, '').substring(0, max) : ''; }
...
const TEMAT_CHATS = [
    ...
    { id: 'temat_sport', name: 'Спорт', avatar: 'https://media1.tenor.com/m/atsrEuZxjdMAAAAd/orange-juice-drinking.gif', description: 'Спортивные новости' }
];
...
// ==================== ЧАСТЬ 3: API АВТОРИЗАЦИЯ И ПОЛЬЗОВАТЕЛИ ====================
...
app.post('/api/users/nickname', (req, res) => {
    ...
    if (!nickname || nickname.length < 3 || nickname.length > 30) {
        return res.status(400).json({ error: 'Никнейм должен быть от 3 до 30 символов' });
    }
...
// Получение темы оформления пользователя
...
app.post('/api/users/theme', (req, res) => {
    ...
    const validThemes = ['default', 'theme-ios', 'theme-whatsapp', 'theme-telegram', 'theme-dark'];
...
app.put('/api/chats/:chatId', (req, res) => {
    ...
    // Проверка прав (только владелец или админ канала)
    let hasRights = false;
    if (chat.type === 'role-based') {
        const role = chat.subscribers?.[userId]?.role;
        hasRights = role === 'admin' || role === 'owner';
    } else if (chat.type === 'school') {
        hasRights = chat.teacher?.id === userId || chat.students?.some(s => s.id === userId);
    } else {
        hasRights = chat.participants?.includes(userId);
    }
    
    if (!hasRights) return res.status(403).json({ error: 'Нет прав' });
    
    const { name, description, avatar, permissions, public: isPublic } = req.body;
...
app.get('/api/channels/public', (req, res) => {
    ...
    const publicChannels = Object.values(chats)
        .filter(chat => (chat.type === 'role-based' && chat.public === true))
        .map(chat => ({
            id: chat.id,
            name: chat.name,
            avatar: chat.avatar,
...
app.post('/api/channels/:channelId/join', (req, res) => {
    ...
    const channel = chats[req.params.channelId];
    if (!channel || channel.type !== 'role-based') {
...
// ==================== ЧАСТЬ 5: API СООБЩЕНИЙ, ФАЙЛОВ, ПОСТОВ, РЕАКЦИЙ ====================

// ============ ПОЛУЧЕНИЕ СООБЩЕНИЙ ЧАТА (С РАСШИФРОВКОЙ) ============
...
// ============ ОТПРАВКА СООБЩЕНИЯ (С ШИФРОВАНИЕМ НА СЕРВЕРЕ) ============
app.post('/api/chats/:chatId/messages', upload.single('file'), (req, res) => {
    ...
    let encrypted = null;
    if (key && (messageType === 'text' || messageType === 'post')) {
    ...
    else if (messageType === 'sticker') lastMessageText = '😊 Стикер';
    else if (messageType === 'post') lastMessageText = `📝 Пост`;
    ...
    // Рассылка через WebSocket всем участникам с расшифровкой для каждого
    ...
    recipients.forEach(pid => {
        if (clients.has(pid)) {
            ...
            if (encrypted && pid !== userId && chat.encryptedKeys?.[pid]) {
                ...
                const decryptedContent = decryptMessage(encrypted, recipientKey, chat.id);
                msgToSend.data.message.content = decryptedContent;
            } else if (encrypted && pid === userId && key) {
                msgToSend.data.message.content = decryptMessage(encrypted, key, chat.id);
            } else {
                msgToSend.data.message.content = messageContent;
            }
    ...
    recipients.forEach(pid => {
        if (clients.has(pid)) {
            clients.get(pid).send(JSON.stringify({
                ...
                data: { chatId: chat.id, messageId: req.params.messageId }
            }));
        }
    });
...
app.post('/api/chats/:chatId/posts', (req, res) => {
    ...
    if (chat.type === 'role-based') {
        ...
        const member = chat.members?.[userId];
        canCreatePost = member?.role === 'admin' || member?.role === 'owner';
    }
    ...
    recipients.forEach(pid => {
        if (clients.has(pid)) {
            clients.get(pid).send(JSON.stringify({
                ...
                data: { chatId: chat.id, postId: post.id, likesCount: Object.keys(post.likes).length }
            }));
        }
    });
...
app.post('/api/chats/:chatId/posts/:postId/comments', (req, res) => {
    ...
    recipients.forEach(pid => {
        if (clients.has(pid)) {
            clients.get(pid).send(JSON.stringify({
                type: 'new_comment',
                data: { chatId: chat.id, postId: post.id, comment }
            }));
        }
    });
...
// ==================== ЧАСТЬ 6: ШКОЛЬНЫЕ ЧАТЫ (ДЗ, ЦДЗ, ТЕСТЫ) ====================

// ============ СОЗДАНИЕ ШКОЛЬНОГО ЧАТА (ЧЕРЕЗ ОТДЕЛЬНЫЙ ЭНДПОИНТ) ============
...
// ============ ПОЛУЧЕНИЕ ТЕКУЩЕГО ЦДЗ (СКРЫВАЕМ ПРАВИЛЬНЫЕ ОТВЕТЫ ДЛЯ УЧЕНИКОВ) ============
...
// ============ ОТПРАВКА ОТВЕТОВ НА ЦДЗ (АВТОМАТИЧЕСКАЯ ПРОВЕРКА) ============
app.post('/api/school/:chatId/quiz/submit', (req, res) => {
    ...
    if (clients.has(userId)) {
        let msgToSend = { type: 'new_message', data: { chatId: chat.id, message: { ...message, content: resultMessage, encrypted: undefined } } };
        if (encrypted && chat.encryptedKeys?.[userId]) {
            const userKey = Buffer.from(chat.encryptedKeys[userId], 'base64');
            msgToSend.data.message.content = decryptMessage(encrypted, userKey, chat.id);
        }
        clients.get(userId).send(JSON.stringify(msgToSend));
    }
    
    res.json({ success: true, score, total: chat.currentQuiz.questions.length, percentage, passed });
});

// ==================== ЧАСТЬ 7: СООБЩЕСТВА, СТИКЕРЫ, ИЗБРАННОЕ, БУФЕР ПЕРЕСЫЛКИ ====================

// ============ СООБЩЕСТВА: ПОЛУЧЕНИЕ СПИСКА ЧАТОВ СООБЩЕСТВА ============
...
app.post('/api/community/:communityId/chats', (req, res) => {
    ...
    Object.keys(community.members || {}).forEach(pid => {
        if (clients.has(pid)) {
            clients.get(pid).send(JSON.stringify({
                type: 'community_chat_created',
                data: { communityId: community.id, chatId, chatName: name }
            }));
        }
    });
...
app.post('/api/stickers/global', upload.single('sticker'), (req, res) => {
    ...
    // Простая проверка на админа (первый пользователь)
...
// ============ СТИКЕРЫ: ИЗБРАННЫЕ (ПОЛЬЗОВАТЕЛЬСКИЕ) ============
...
// ==================== ЧАСТЬ 8: WEBSOCKET, ЗАКРЕПЛЕНИЕ ПОСТОВ, СТАТИКА, ЗАПУСК ====================
    ...
    ws.on('message', async (data) => {
        try {
            ...
            switch (type) {
                    ...
                    if (chat.participants) {
                        chat.participants.forEach(pid => {
                            if (pid !== userId && clients.has(pid)) {
                                clients.get(pid).send(JSON.stringify({
                                    type: 'typing',
                                    data: { chatId, userId, userName: users[userId]?.username }
                                }));
                            }
                        });
                    }
                ...
                case 'read':
                    // Отметка о прочтении (можно реализовать позже)
                    break;
                
                case 'reaction':
                    if (messageId && messages[chatId]) {
                            ...
                            recipients.forEach(pid => {
                                if (clients.has(pid)) {
                                    clients.get(pid).send(JSON.stringify({
                                        ...
                                        data: { chatId, messageId, reactions: messages[chatId][msgIndex].reactions }
                                    }));
                                }
                            });
                        }
...
app.post('/api/chats/:chatId/posts/:postId/pin', (req, res) => {
    ...
    // Обновляем список закреплённых постов в чате
    if (!chat.pinnedPosts) chat.pinnedPosts = [];
    if (messages[chat.id][postIndex].pinned) {
        if (!chat.pinnedPosts.includes(req.params.postId)) {
            chat.pinnedPosts.push(req.params.postId);
        }
    } else {
        chat.pinnedPosts = chat.pinnedPosts.filter(id => id !== req.params.postId);
    }
...
// ============ ПОЛУЧЕНИЕ ЗАКРЕПЛЁННЫХ ПОСТОВ ============
app.get('/api/chats/:chatId/pinned', (req, res) => {
    const token = req.headers.authorization?.split(' ')[1];
    const userId = verifyToken(token);
    if (!userId) return res.status(401).json({ error: 'Не авторизован' });
    ...
    const pinnedPosts = (chat.pinnedPosts || [])
        .map(postId => messages[chat.id]?.find(m => m.id === postId))
...
server.listen(PORT, () => {
    console.log('\n╔═══════════════════════════════════════════════════════════╗');
    console.log('║     🔒 OKSAGRAM FULL SERVER STARTED 🔒                   ║');
    console.log('╠═══════════════════════════════════════════════════════════╣');
    console.log(`║     📍 Local:    http://localhost:${PORT}                   ║`);
    console.log(`║     🌍 Network:  http://0.0.0.0:${PORT}                    ║`);
    console.log('╠═══════════════════════════════════════════════════════════╣');
    console.log('║     ✅ Функционал:                                         ║');
    console.log('║     📱 Личные чаты | 👥 Группы | 📢 Каналы                 ║');
    console.log('║     🏫 Школьные чаты | 🌐 Сообщества | 🔥 Тематы           ║');
    console.log('║     📝 Посты с лайками | 💬 Комментарии                    ║');
    console.log('║     😊 Стикеры | ⭐ Избранное | 📦 Буфер пересылки         ║');
    console.log('║     🔐 AES-256-GCM шифрование | 🔑 JWT авторизация        ║');
    console.log('╠═══════════════════════════════════════════════════════════╣');
